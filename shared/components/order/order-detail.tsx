  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="font-bold text-lg text-gray-800" dir="ltr">
      ...
      <div className="bg-white rounded-lg shadow p-4 border border-[#A9CBF5]/30">
        ...
        <div className="text-sm text-gray-700 mb-2">
          مبلغ کل: <span className="font-bold">{order.totalAmount} تومان</span>
        </div>
        {order.discountAmount > 0 && (
          <div className="text-sm text-gray-700 mb-2">
            تخفیف:{" "}
            <span className="font-bold">{order.discountAmount} تومان</span>
          </div>
        )}
        <div className="text-sm text-gray-700 mb-2">
          مبلغ نهایی:{" "}
          <span className="font-bold">{order.finalAmount} تومان</span>
        </div>
        {order.couponCode && (
          <div className="text-sm text-gray-700 mb-2">
            کد تخفیف: <span className="font-medium">{order.couponCode}</span>
          </div>
        )}
        ...
        {order.shippingAddress &&
          typeof order.shippingAddress.detail === "string" && (
            <div className="text-sm text-gray-700 mb-2">
              آدرس:{" "}
        ...
        <ul className="divide-y divide-gray-100">
          {order.items?.map((item) => (
            <li key={item.id} className="py-2 flex items-center gap-3">
              ...
              <div className="font-bold text-sm text-[#1473E6]">
