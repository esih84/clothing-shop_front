...
export function ProductCard({
  ...
  return (
    <Link href={`/product/${id}`} className="block group" style={{ viewTransitionName: `product-${id}` }}>
      ...
      <div>
        <h3 className="font-medium text-base mb-1" style={{ viewTransitionName: `product-title-${id}` }}>
          {title}
        </h3>
        <div className="flex items-center gap-1 mb-1">
          <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
          <span className="text-xs font-medium">{rating}</span>
          <span className="text-xs text-gray-500">({reviews.toLocaleString()} sold)</span>
        </div>
        <p className="font-bold" style={{ viewTransitionName: `product-price-${id}` }}>
          ${price.toFixed(2)}
        </p>
      </div>
    </Link>
